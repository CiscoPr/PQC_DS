...
RESULTS_DIR = f"/results/general/mode{mode}/"
CSV_FILE = os.path.join(RESULTS_DIR, f"dilithium_times_mode{mode}.csv")
IMG_FILE = os.path.join(RESULTS_DIR, f"dilithium_times_mode{mode}.png")
...
def generate_graph():
    ...
    ax1.plot(df['run'], df['keygen'], marker='o', color='tab:blue', label='KeyGen (ms)')
    ax1.plot(df['run'], df['sign'], marker='x', color='tab:orange', label='Sign (ms)')
    ax1.plot(df['run'], df['verify'], marker='s', color='tab:green', label='Verify (ms)')
